...
for (x in 1: length(sample_dates) - 1){
  dif <- interval(from = as.Date(sample_dates[x]), to = as.Date(sample_dates[x+1]))
  print(month(dif))
  print(weeks(dif))
}
```
...
get_month_name <- possibly(function(date_strings){
  dashed_date <- as.Date(date_strings, format = "%Y-%m-%d")
  if dashed_date is.na()
  slash_date <- as.Date(date_strings, format = "%Y/%m/%d")
  if slash_date is.na()
  word_date <- as.Date(date_strings, format = "%d, %b, %Y")
